...
describe("CLI providers — not reachable via getProviderConfig", () => {
  it.each([
    ["claude-code", "claude-sonnet-4-6"],
    ["codex-cli", "gpt-5.5"],
  ] as const)("throws for %s, because subprocess transport dispatches one layer up in streamChat", (provider, model) => {
    // If this ever stops throwing, someone wired a CLI provider into
    // the HTTP path by mistake — they have no URL/headers and would
    // crash silently inside fetch() otherwise.
    expect(() =>
      getProviderConfig({
        provider,
        apiKey: "",
