...
type RejectMode =
  | { kind: "accept-origin"; origin: string }
  | { kind: "reject-all" }

/**
 * Start a fake Ollama-compatible server on 127.0.0.1. Behavior is
 * scriptable per test:
 *   - "accept-origin": 200 + SSE stream when the request's
 *     Origin equals the configured origin; 403 with a body
  ...
  const url = await new Promise<string>((resolve, reject) => {
    server = createServer((req: IncomingMessage, res: ServerResponse) => {
      ...
      req.on("end", () => {
        ...
        const allow =
          mode.kind === "accept-origin" && origin === mode.origin
        if (!allow) {
          res.statusCode = 403
...
describe("streamChat against a fake Ollama server (real TCP)", () => {
  ...
  it(
    "the fake server's CORS check rejects when localhost is not in the allowlist",
    async () => {
      // Sanity check: if our Origin override regressed, the server
      // would stop exercising this branch. We force a wrong-origin
      // scenario by setting the server to accept only its own URL,
      // while the client intentionally sends http://localhost, and confirm
      ...
      try {
        server.setRejectMode({ kind: "accept-origin", origin: server.url })
